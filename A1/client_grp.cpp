...
/**
 * @file client_grp.cpp
 * @brief Tiny chat client that talks to the group server (ELI5 comments)
 *
 * What this code does:
 *  - Connects to the server on PORT.
 *  - Lets you type commands (/msg, /broadcast, etc.) and sends them to server.
 *  - Prints anything the server sends back.
 *  - Uses select() to watch both STDIN and the socket at the same time so the
 *    app feels responsive without threads.
 */
