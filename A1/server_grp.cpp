/**
 * @file server_grp.cpp
 * @brief Group‑chat server (explained like you're new to sockets)
 *
 * What this program does (big picture):
 *  - Starts a TCP server that multiple clients can connect to.
 *  - Asks each client for a username + password (from users.txt) and logs them in.
 *  - Lets logged‑in clients:
 ...
 *      /group_msg <name> <text>   → message everyone in a group
 *      CLOSE                      → disconnect
 *
 * Why the code looks like this:
 *  - We use epoll (Linux) to handle many sockets without making one thread per
 *    client. Think of epoll as a very fast “doorbell” that rings when a socket
 *    has data to read or can be written.
 *  - We keep a tiny “session state” for each client: are they picking a
 *    username? entering a password? or fully authenticated?
 *  - Data structures are named after how you’d describe them out loud:
 *     • usernameTofd / fdTousername map between names and sockets.
 *     • groupTofd keeps a set of client fds for each group name.
 *
 * Read this file top‑to‑bottom and skim the comments. Nothing fancy is hidden;
 * every moving part is explained at the moment it appears.
 */
